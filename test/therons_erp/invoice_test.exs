defmodule TheronsErp.InvoiceTest do
  ...
  test "copying line items" do
    ...
    product = Inventory.create_product!("Test Product", Money.new(100, :USD), %{cost: Money.new(50, :USD)})
    sales_line =
      SalesLine
      |> Ash.Changeset.for_create(:create, %{sales_order_id: sales_order.id, product_id: product.id, quantity: 1})
      |> Ash.create!()

    invoice =
      Invoice
      |> Ash.Changeset.for_create(:create, %{sales_order_id: sales_order.id, sales_lines: [sales_line]})
      |> Ash.create!()
      |> Ash.load!(:line_items)
